...
def key_accending_order(x,y):
    return 1 if x["fruit"]>y["fruit"] else -1

def key_decending_order(x,y):
    return 1 if x["fruit"]<y["fruit"] else -1
...
sortdict = sorted(new_dict, key=cmp_to_key(key_accending_order))
second_sortdict = sorted(new_dict, key=cmp_to_key(key_decending_order))
print(sortdict)
